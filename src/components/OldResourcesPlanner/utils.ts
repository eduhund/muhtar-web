...
export const calculateMetrics = (
  ...
  if (!item.children || item.children.length === 0) {
    ...
    // Aggregate assignments from all children
    const assignmentsMap = new Map<string, ProjectAssignment>();
    childMetrics.forEach((cm) => {
      cm.assignments.forEach((assignment) => {
        const key = `${assignment.worker}-${assignment.role}`;
        if (!assignmentsMap.has(key)) {
          assignmentsMap.set(key, assignment);
        }
      });
    });
    const assignments: ProjectAssignment[] = Array.from(
