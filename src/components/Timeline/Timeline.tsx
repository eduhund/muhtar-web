...
import { flattenData, getRowHeight, getWeekNumber } from "./utils";
...
export function Timeline({
  ...
  const handleStatusChange = useCallback(
    (itemId: string, newStatus: TaskStatus) => {
      ...
      const updateItem = (item: GanttItem): GanttItem => {
        if (item.id === itemId) {
          const updatedItem = { ...item, status: newStatus };

          // Handle jobs updates based on new status
          if (item.jobs && item.jobs.length > 0) {
            if (newStatus === "inProgress") {
              ...
              if (completedCount === item.jobs.length) {
                // 1.2.4 All jobs completed - set last child to inProgress
                updatedItem.jobs = item.jobs.map((child, index) => {
                ...
                updatedItem.jobs = item.jobs.map((child) => {
              ...
              // 2.2 Set all jobs to completed, except canceled
              const setChildrenCompleted = (child: GanttItem): GanttItem => ({
                ...child,
                status: child.status === "canceled" ? "canceled" : "completed",
                jobs: child.jobs?.map(setChildrenCompleted),
              });
              updatedItem.jobs = item.jobs.map(setChildrenCompleted);
            } else if (newStatus === "canceled") {
              // 3.2 Set all jobs to canceled
              const setChildrenCanceled = (child: GanttItem): GanttItem => ({
                ...child,
                status: "canceled",
                jobs: child.jobs?.map(setChildrenCanceled),
              });
              updatedItem.jobs = item.jobs.map(setChildrenCanceled);
            } else if (newStatus === "backlog") {
              // 4.2 Set all jobs to backlog
              const setChildrenBacklog = (child: GanttItem): GanttItem => ({
                ...child,
                status: "backlog",
                jobs: child.jobs?.map(setChildrenBacklog),
              });
              updatedItem.jobs = item.jobs.map(setChildrenBacklog);
            }
          }
        ...
        if (item.jobs) {
          const updatedJobs = item.jobs.map((child) => updateItem(child));

          // Check if this item is parent of the changed item
          const hasChangedChild = updatedJobs.some((c) => c.id === itemId);

          if (hasChangedChild) {
            const childStatuses = updatedJobs.map((c) => c.status || "backlog");
            ...
            if (newStatus === "inProgress") {
              ...
              // 2.3 If all jobs completed - parent becomes completed, else inProgress
              if (allCompleted) {
